...
### `Cannot find module 'src/...'` when running a script

`src/users/users.module.ts:14` imports `'src/ads/schemas/ad.schema'` — an absolute
specifier resolved through `baseUrl: "./"`. Every other import in that file is
relative, so this one is a stray.

It does not break the app: `nest build` rewrites path aliases in its output
(`dist/users/users.module.js` correctly contains `require("../ads/schemas/ad.schema")`),
so `pm2-runtime start ecosystem.config.js` → `dist/main.js` is unaffected. Bare
`ts-node` is the only thing that trips over it, because it does not apply
`baseUrl` on its own.

Fixed for the search scripts by running them through `tsconfig-paths`:

```
ts-node -r tsconfig-paths/register src/search/scripts/search-preflight.ts
```

**Every other `ts-node` script in package.json has the same latent problem** and
will fail the same way the first time it is run against a module graph that
reaches `users.module.ts`:

`seed:ads:enhanced`, `chat:backfill`, `chat:smoke`, `chat:indexes`,
`seed:manufacturers:safe`, `seed:fuel-types:safe`, `seed:transmission-types:safe`,
`seed:vehicle-models:safe`, `seed:vehicle-variants:safe`, `add:superadmin`,
`create:superadmin`, `fix:commercial-vehicle-data`.

Two ways to close it properly, neither urgent:

1. Add `-r tsconfig-paths/register` to those scripts too (zero risk — dev tooling only).
2. Make the one import relative (`../ads/schemas/ad.schema`) and drop the need
   entirely. This touches a file in the application boot path, so it wants a
   normal build-and-deploy rather than a quiet edit.

### `querySrv ECONNREFUSED _mongodb._tcp.<cluster>.mongodb.net`

The `mongodb+srv` lookup cannot resolve. The workaround for it (force IPv4,
override the resolver) was written at module scope in `src/main.ts`, so it
applied **only when `main.ts` was the entry point** — every script that
bootstraps `AppModule` directly got no DNS fix at all.

Extracted to `src/common/database/dns-bootstrap.ts` and called from `main.ts`
and from each search script, so the two cannot drift. Behaviour is unchanged and
it is still a no-op when `NODE_ENV=production`.

If it still fails, the resolver defaults (`8.8.8.8,1.1.1.1`) are being blocked —
override them:

```
DNS_SERVERS=1.1.1.1,9.9.9.9 npm run search:preflight
```

Any new script that boots `AppModule` must call `applyLocalDnsWorkaround()`
before `NestFactory`, because Mongoose connects during module initialisation.
