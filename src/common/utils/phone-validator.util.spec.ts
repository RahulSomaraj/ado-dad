...
describe('Phone Validation Utility', () => {
  describe('validatePhoneNumber', () => {
    ...
    it('should validate US phone numbers correctly', () => {
      // US numbers need area code starting with 2-9
  ...
  describe('validatePhoneNumberWithPhoneCode', () => {
    ...
    it('should validate with phone code +1', () => {
      // US numbers need area code starting with 2-9
      const result = validatePhoneNumberWithPhoneCode('2025551234', '+1');
      expect(result.valid).toBe(true);
    });
  ...
  describe('formatPhoneNumber', () => {
    ...
    it('should format US phone number', () => {
      const formatted = formatPhoneNumber('US', '2025551234');
  ...
  describe('parsePhoneNumber', () => {
    ...
    it('should parse full US phone number', () => {
      ...
      expect(parsed).not.toBeNull();
      // +1 can map to multiple countries, so just check it's not null
      expect(parsed?.countryCode).toBeDefined();
      expect(parsed?.phoneNumber).toBeDefined();
    });
  ...
  describe('phoneCodeToIsoCode', () => {
    ...
    it('should convert +1 to a valid country', () => {
      // +1 is shared by multiple countries (US, Canada, etc.)
      const isoCode = phoneCodeToIsoCode('+1');
      expect(isoCode).toBeDefined();
      expect(['US', 'CA', 'AG', 'AI', 'AS', 'BB', 'BM', 'BS', 'DM', 'DO', 'GD', 'GU', 'JM', 'KN', 'KY', 'LC', 'MP', 'MS', 'PR', 'TC', 'TT', 'VC', 'VG', 'VI']).toContain(isoCode);
    });
